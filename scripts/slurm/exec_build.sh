...
cat sstdbg.jenkins.${SLURM_ID}.out

#-- job has completed, test for status
STATE=`cat sstdbg.jenkins.${SLURM_ID}.out | grep "tests failed out of"`
NUM_FAILED=`cat sstdbg.jenkins.${SLURM_ID}.out | grep "tests failed out of" | awk '{print $4}'`
